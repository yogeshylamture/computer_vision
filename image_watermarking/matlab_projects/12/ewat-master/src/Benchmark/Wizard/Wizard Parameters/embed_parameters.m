function varargout = embed_parameters(varargin)

        gui_Singleton = 1;
        gui_State = struct('gui_Name',       mfilename, ...
                           'gui_Singleton',  gui_Singleton, ...
                           'gui_OpeningFcn', @embed_parameters_OpeningFcn, ...
                           'gui_OutputFcn',  @embed_parameters_OutputFcn, ...
                           'gui_LayoutFcn',  [] , ...
                           'gui_Callback',   []);
...
%% Opening function
function embed_parameters_OpeningFcn(hObject, eventdata, handles, varargin)

        handles.output = hObject;
        guidata(hObject, handles);
        
        number_of_embed_arguments = get_root_data('number_of_embed_arguments');
        if number_of_embed_arguments == 1
        ...
        embed_parameters = struct();
        set_root_data(handles,'embed_parameters',embed_parameters);
        
        set_root_data(handles,'current_argument',current_argument);
        arguments = int2str(number_of_embed_arguments);
        set_ui(handles,'initialize');
        set(handles.tab_parameter, 'Title', ['Embedding paramater 1 of ',arguments]);
        
%% Varargout
function varargout = embed_parameters_OutputFcn(hObject, eventdata, handles) 
...
if strcmp(next,'Next')
    ...
    arguments = get_root_data('number_of_embed_arguments');
    ...
    embed_parameters = setfield(embed_parameters, parameter_name,parameter_value);
    set_root_data(handles,'embed_parameters',embed_parameters);
    ...
    if current_argument == arguments
        ...
        iMark_script = setfield(iMark_script,'embed_parameters',embed_parameters);
