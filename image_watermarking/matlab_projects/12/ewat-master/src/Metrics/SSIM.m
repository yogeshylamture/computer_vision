...
%if the image are not single-layer reshape them
    [img1_H,img1_W,img1_D] = size(img1);
    if img1_D ~= 1 && img1_D == 4   %transparant layer present
        img1 = reshape(img1(:,:,1:3),[img1_H (img1_D-1)*img1_W]);
    else
        img1 = reshape(img1,[img1_H (img1_D)*img1_W]);
    end

    [img2_H,img2_W,img2_D] = size(img2);
    if img1_D ~= 1
        img2 = reshape(img2,[img2_H img2_D*img2_W]);
    else
    end
