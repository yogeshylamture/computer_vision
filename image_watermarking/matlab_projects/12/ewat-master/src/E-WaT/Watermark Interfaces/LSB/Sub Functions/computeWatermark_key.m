...
function computeWatermark_key(handles, messageFile, type, cover, resize)

attack = get_root_data('attack');
switch attack 
    case 'no'
        keyType = get_root_data('chosen_key');
        switch keyType
            case 'text'
                keyFile = get_root_data('chosen_key_text');
                key = sum(keyFile);
            case 'imag'
                keyFile = get_root_data('chosen_key_image');
                key = sum(sum(sum(keyFile)));
        end;
    otherwise
        keyType = get_root_data('chosen_key_attack');
        switch keyType
            case 'text'
                keyFile = get_root_data('chosen_key_text_attack');
                key = sum(keyFile);
            case 'imag'
                keyFile = get_root_data('chosen_key_image_attack');
                key = sum(sum(sum(keyFile)));
        end;
end;
    ...
    case 'no'
        switch type
            case 'imag'                                                            %message is an image file
                ...
                watermark = im2bw(randn(cH, cW));                                             %create empty watermark (ones because 1 = white)
                ...
                randn('state', key);
                M_0 = im2bw(randn(blocksize*factor));
            ...
            case 'text'                                                               %message source file is a text file
                ...
                if(nrR >= nrR_needed && cW >= aW*nrC)                                 %if cover work is big enough for message (message is max when factor is one)
                    ...
                    temp = im2bw(randn(ceil(cH/factor), ceil(cW/factor)));         %create temproy noise matrix based on key
                    for v = 1:cH                                                  %copy date from temp into watermark (take each value of temp factor times in watermark)
                        for w = 1:cW                                              %we do this to generate noise with same resolution for 1 black or white 'pixel'
                            watermark(v,w) = temp(ceil(v/factor), ceil(w/factor));
                        end
                    end
                    ...
                    for i=1:nrR_needed
                        if (i == nrR_needed)                                      %last row
                            ...
                            for j=1:nrC_last_row
                                ...
                                m = get_pattern(ind, patterns);                                %original matrix represenataion
                            ...
                            for j=1:nrC
                                ...
                                m = get_pattern(ind, patterns);                                %M = new (larger) matrix representation of the characters depending on factor
                                for v = 1:factor*aH                               %copy date from m into M (take each value of m factor times in M)
                                    for w = 1:factor*aW
                                        M(v,w) = m(ceil(v/factor), ceil(w/factor));
                                    end
