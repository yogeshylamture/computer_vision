...
switch watermark_found
    case 'on'       % Text was the message
        ...
        L = length(text_embed);
        messages = strvcat(strtrim(get(handles.detMessTxt1, 'String')),... 
        ...
        match = zeros(1,8);
        for i=1:8
            match(i) = strncmpi(text_embed, messages(i,:),L);
        end        
        [C, I] = max(match);
        if (C ~= 0)             %complete match (messages identical)
            ...
            match_letter = zeros(8,L);
            for i=1:8
                for j=1:L
                    if (strncmpi(messages(i,j), text_embed(j), 1))
                        match_letter(i,j) = 1;
                    end
                end
            end
